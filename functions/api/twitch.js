export async function onRequest(context) {
  ...
  const CHANNEL_LOGIN = String(
    env.TWITCH_CHANNEL_LOGIN || "yuzukosyo07"
  ).trim();
    ...
    if (!tokenResponse.ok) {
      ...
      return json({
        ...
        scheduleMessage:
          "Twitchトークン取得エラー",
        error:
          `Twitch token error: ${tokenResponse.status} ${text}`
      });
    ...
    if (!userResponse.ok) {
      ...
      return json({
        ...
        scheduleMessage:
          "Twitchユーザー取得エラー",
        error:
          `Twitch user error: ${userResponse.status} ${text}`
      });
    ...
    if (!liveResponse.ok) {
      ...
      return json({
        ...
        scheduleMessage:
          "ライブ情報取得エラー",
        error:
          `Twitch live error: ${liveResponse.status} ${text}`
      });
